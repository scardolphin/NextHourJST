  ...
  switch (command) {
    case "starttime":
      if (message.channel.timeout) return message.channel.send ('time is already started, use !stoptime to stop the loop.');
            ...
            message.channel.send(''+new Date().toLocaleTimeString("jp-JP",{timeZone:"Asia/Tokyo",hour12:false}));
       message.channel.loop = setInterval (() => message.channel.send (' '+ new Date().toLocaleTimeString("jp-JP",{timeZone:"Asia/Tokyo",hour12:false})), sec * 1000)
    }, leftToFiftyFive())
